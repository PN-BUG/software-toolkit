...
- **移动端适配**：支持手机安全区、触控按钮、横向设备列表和底部弹窗
- **双向传输**：浏览器 ↔ PC 之间互传文件
- **实时传输列表**：上传、聊天文件和下载均显示进行中状态，上传显示百分比和速度
- **目录浏览**：网页化文件列表，自动显示文件大小 / 修改时间，支持子目录
- **多端访问**：可同时被多台设备访问
- **设备发现**：自动发现同一局域网内运行中的共享服务，也可手动扫描
- **连接请求**：记住连接过的设备，支持从最近设备列表主动请求重新连接
- **后台唤醒（可选）**：安卓安装独立“蒲公英降落台”后，浏览器关闭时也能收到连接通知，同意后自动打开共享网址
...
默认共享整个 SoftwareToolkit 发布包根目录（例如 `release/SoftwareToolkit-win-x64-standalone/`），即使以管理员身份启动也不会落到 `C:\Windows\System32`。
...
```powershell
# 共享 SoftwareToolkit 发布包根目录,默认端口 8088
.\lan-share.bat
...
网页右上角的“🌱 蒲公英降落台”会打开专用下载页。可点击把 APK 下载到当前设备，也可让安卓手机扫码下载。安装后打开一次并允许通知；蒲公英降落台会以前台服务监听同一局域网中的请求。其他设备点击该手机旁的连接按钮后，手机会弹出系统通知，点击“同意并打开”即可自动进入共享网页。

蒲公英降落台不是文件共享服务的必需组件：不安装它时，扫码、上传、下载、聊天等原有功能不受影响。

## 🛠 高级参数

| 参数 | 说明 | 默认值 |
|------|------|--------|
| `-SharePath` | 共享根目录 | SoftwareToolkit 发布包根目录；独立复制脚本时为脚本目录 |
| `-Port` | HTTP 监听端口 | `8088` |
| `-ReadOnly` | 只允许下载,禁用上传 | `false` |
| `-Token` | 简易访问令牌,带 `?t=xxx` 才能访问 | 无 |
| `-NoBrowser` | 启动服务但不自动打开浏览器，适合测试或后台运行 | `false` |
